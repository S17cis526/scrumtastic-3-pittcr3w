...
Router.prototype.resource = function(route, resource) {
  ...
  var db = this.db;
  if (resource.list) this.get(route, function(req, res) {project.list(req, res, db)});
  if (resource.create) this.post(route, function(req, res){project.create(req, res, db)});
  if (resource.read) this.get(route + '/:id', function(req, res){project.read(req, res, db)});
  if (resource.update) this.post(route + '/:id', function(req,res){project.update(req, res, db)});
  if (resource.destroy) this.get(route + '/:id/destroy', function(req, res){project.destroy(req,res, db)});
